...
describe("ConfigYAMLLoader", function() {
  ...
  it("should be able to parse a YAML based config file", function() {
    ...
    expect(config.instances[INSTANCE1_KEY].description).to.equal(
      INSTANCE1_DESCRIPTION
    );
    ...
    expect(config.instances[INSTANCE1_KEY].creds.username).to.equal(
      INSTANCE1_USER
    );
    expect(config.instances[INSTANCE1_KEY].creds.password).to.equal(
      INSTANCE1_PASS
    );
  });
