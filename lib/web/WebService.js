...
// Now the route has been set see if route requires auth and if so lookup strategy and invoke
...
app.get('/rest/plugins/:pluginKey/:moduleKey/*', function (req, res) {
  ...
  if(req.params[0]){
    console.log("additional rest params: ", req.params[0]);
    url += "/" + req.params[0];
  }
...
app.all('/plugins/:pluginKey/webpages/:moduleKey', function (req, res) {
  ...
  console.log("proxy method is: ", req.method);
  var plugin = PluginService.getPluginByKey(req.params.pluginKey);
