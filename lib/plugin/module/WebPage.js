...
var checkAndApplyWebFragments = function(template, pluginManager){
      ...
      if( allWebfragments && allWebfragments.length > 0 ){
        allWebfragments.forEach(function(webfragment){
          if(webfragment.getLocation() == location){
            fragmentsToReplace += webfragment.render();
          }
        });
      };
...
class WebPage extends Module{
  ...
  render(){
    ...
    template = checkAndApplyWebFragments(template, pluginManager);
