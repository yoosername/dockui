...
class HTMLResource{
  ...
  parseHtml(html){
    ...
    try{
      ...
      console.log("[WebPage Module:HTMLResource] : Error parsing HTML : ", error);
      return;
    }
    ...
    if( type == null ){
      console.log("[WebPage Module:HTMLResource] : Couldnt parse Resource HTML: Not a valid type of (link, style, script)");
      return;
    }
  ...
  html(){
    if( this.type ){
      ...
      console.log("[WebPage Module:HTMLResource] : Resource has no type, cant transform into HTML: ", this.json());
    }
  }
