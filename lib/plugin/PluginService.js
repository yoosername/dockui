 ...
 */
 class PluginService extends EventEmitter{
   ...
   addPlugin(containerId, port, network){
     ...
     // TODO: add regular status page check so plugins can reload themselves ( without container restart )
     // TODO: potentially move the fetch code into the Plugin definition.

     // Get status page once and then every 5 seconds
     //  - if unavailable for 10 seconds remove plugin
     //  - if is available but uptime changed then reload plugin
     // then
