 ...
 */
 class AuthService extends EventEmitter{
   ...
   authenticate(req){
     console.log("[AuthService] : Attempting to Authenticate request");
     var providerId = 0;
 ...
 var authenticateByProvider = function(req, providerId, providers){
   ...
   return new Promise(function(resolve, reject){
       ...
       if( providers ){
         ...
         console.log("[AuthService] : Trying provider %d of %d", currentProviderId+1, providers.length);
         providers[currentProviderId].authenticate(req)
            ...
            .catch(function(error){
              ...
              else if( error instanceof Errors.ProviderCantHandleAuthenticationRequestError ){

                var nextProviderId = currentProviderId+1;
                if( providers[nextProviderId] ){
                  resolve( authenticateByProvider(req, nextProviderId, providers) );
                }else{
